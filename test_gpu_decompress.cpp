...
int main() {
  ...
  for (auto i = 0u; i < 90750; i++) {
    ...
    packedMetadata[i] = {0, 0, 0};
    hipMemcpy(byte_input + 11 * i, &val, 8);
    val = 3;
    packedMetadata[i + 1] = {1, 1, 48};
    hipMemcpy(byte_input + 11 * i + 8, &val, 1);
    val = 231;
    packedMetadata[i + 2] = {1, 1, 47};
    hipMemcpy(byte_input + 11 * i + 9, &val, 1);
    val = 29;
    packedMetadata[i + 3] = {1, 1, 47};
    hipMemcpy(byte_input + 11 * i + 10, &val, 1);
  }
